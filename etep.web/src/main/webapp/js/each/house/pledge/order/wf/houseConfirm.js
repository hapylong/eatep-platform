...
$package('IQB.houseConfirm');
IQB.houseConfirm = function(){
	var _this = {
		cache: {
			dictTypeCode: {
...
//			}
//			var option = {variableData: {procAuthType: '2', procApprStatus: procApprStatus, procApprOpinion: procApprOpinion}, bizData: {procBizId: window.procBizId}, procData: {procTaskId: window.procTaskId}};
//			IQB.post(urls['house'] + '/WfTask/completeProcess', option, function(result){
//				var url = window.location.pathname;
//				var param = window.parent.IQB.main2.fetchCache(url);
//				var callback = '_this.clickCloseTab(\'' + param.tabId + '\')';
...
//			});
		...
		initOrderInfo: function(){
			...
			IQB.post(urls['house'] + '/order/getOrderByBusiness', {businessId: window.procBizId}, function(result){
				...
				$('#houseDesc').text(Formatter.ifNull(result.iqbResult.result.houseDesc) + '万元');//备注
			});
		},
