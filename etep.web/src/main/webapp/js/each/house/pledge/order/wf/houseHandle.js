...
//procBizId = '2016-11-8-10000';

$package('IQB.houseHandle');
IQB.houseHandle = function(){
	var _this = {
		cache: {
			dictTypeCode: {
...
//			var option = {variableData: {procAuthType: '2', procApprStatus: '1', procApprOpinion: '同意'}, bizData: {procBizId: window.procBizId}, procData: {procTaskId: window.procTaskId}};
//			IQB.post(urls['house'] + '/WfTask/completeProcess', option, function(result){
//				var url = window.location.pathname;
//				var param = window.parent.IQB.main2.fetchCache(url);
//				var callback = '_this.clickCloseTab(\'' + param.tabId + '\')';
...
//			});
		...
		initOrderInfo: function(){
			...
			IQB.post(urls['house'] + '/order/getOrderByBusiness', {businessId: window.procBizId}, function(result){
				...
				//处理上传抵押通知书
				if(result.iqbResult.result.businessType == 10){
				...
				$.each(result.iqbResult.result, function(i, n){
					...
					if(Formatter.extensionName.doc.contain(extensionName)){
						...
						var html = '<div class="thumbnail float-left" style="width: 145px;">' + 
						      			...
						      			'<div class="caption">' +
						      				'<h5>' + n.fileName + '</h5><h6><a filePath="' + n.filePath + '" onclick="IQB.houseHandle.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
						      			'</div>' + 
						      		'</div>';
				...
				$.each(files, function(i, n){
					...
					html += '<div class="thumbnail float-left" style="width: 145px;">' + 
					      		'<a href="javascript:void(0)"><img src="' + urls['imgUrl'] + option.filePath + '" data-original="' + urls['imgUrl'] + option.filePath + '" alt="' + option.fileName + '" style="width: 135px; height: 135px;"></a>' +
					      		 '<div class="caption">' +
					      		 	'<h5>' + option.fileName + '</h5><h6><a filePath="' + option.filePath + '" onclick="IQB.houseHandle.removeFile(event);"><span class="glyphicon glyphicon-trash"></span></a></h6>' +
					      		 '</div>' + 
					      	 '</div>';
				});
