package com.iqb.eatep.ec.contract.ssq.service.impl;
...
import com.iqb.eatep.ec.util.EncodeUtil;
...
public class BestSignServiceImpl extends BestSignService {	
    ...
    public JSONObject regUser(Map parameterMap) throws IqbException {    	
    	try {
    	    String userType = ObjectUtils.toString(parameterMap.get("ecSignerType"));
        	if("1".equals(userType)){
                ...
                Map<String, Object> resultMap = this.regUser(userType, ecSignerPhone, ecSignerName);
                return parseResultMap(resultMap);
        	}
        	if("2".equals(userType)){
        	    verifyItemOfMap(parameterMap, new String[] {"ecSignerType", "ecSignerPhone", "storeName", "ecSignerEmail"}, true);
                String ecSignerPhone = (String) parameterMap.get("ecSignerPhone");
                String ecSignerName = (String) parameterMap.get("storeName");        
                String ecSignerEmail = (String) parameterMap.get("ecSignerEmail");        
                Map<String, Object> resultMap = this.regUser(userType, ecSignerPhone, ecSignerName, ecSignerEmail);
                return parseResultMap(resultMap);
        	}
	...
	@Override
	protected Map<String, Object> regUser(final String userType, final String userMobile, final String userName, final String email) throws IqbException, Exception {	
	    final String method = "regUser.json";
	    String path = "/open/" + method;
	    logger.info("上上签注册用户服务：{}", ecConfig.getSsqHost() + path);
	...
	public JSONObject uploadUserImage(Map parameterMap)
		...
		try {
            ...
            if("2".equals(userType)){
                verifyItemOfMap(parameterMap, new String[] {"ecSignerType", "ecSignerPhone", "storeName", "ecSignerImgDataBlob"}, true);
	...
	public JSONObject certificateApplyForEnterprise(Map parameterMap)
		...
		try {
            ...
            if("2".equals(userType)){
                ...
                verifyItemOfMap(parameterMap, new String[] {"ecSignerPhone", "storeName", "ecSignerCertPwd", "ecSignerCertNo", "bizRegNum", "organizationCode", "taxRegNum", "ecSignerProvince", "ecSignerCity", "ecSignerAddress"}, true);
	...
	public JSONObject contractUpload(Map parameterMap)
		...
		try {
	    	...
	    	String ecSignerType = ObjectUtils.toString(sendUser.get("ecSignerType"));
			...
			sendUser.clear();
			sendUser.put("usertype", ecSignerType);
	    	...
	    	if (receiveUserList != null && receiveUserList.size() > 0) {
	    		for (Map<String, Object> m : receiveUserList) {
	        		...
	        		String ecSignerTyp = (String) m.get("ecSignerType");
		...
		try {
	    	...
	    	if (receiveUserList != null && receiveUserList.size() > 0) {
	    		for (Map<String, Object> m : receiveUserList) {
	        		...
	        		String ecSignerType = (String) m.get("ecSignerType");
	...
	public JSONObject autoSign(Map parameterMap) throws IqbException {
		try {
			...
			String ecSignerPhone = (String) parameterMap.get("ecSignerPhone");
	    	String tpSignid = (String) parameterMap.get("tpSignid");
	    	if(!StringUtils.isEmpty(ecSignerEmail)){
	    	    ecSignerPhone = ecSignerEmail;
	    	}
	    	@SuppressWarnings("unchecked")
	    	...
	    	String sealName = (String) getItemOfMap(parameterMap, "ecSignerImgName");
